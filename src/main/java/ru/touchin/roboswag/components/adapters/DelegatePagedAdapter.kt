 ...
 * Adapter based on [PagedListAdapter] and providing some useful features like:
...
class DelegatePagedAdapter<TItem, TItemViewHolder : BindableViewHolder>(
        val stopable: Stopable,
        diffCallback: DiffUtil.ItemCallback<TItem>) : PagedListAdapter<TItem, TItemViewHolder>(diffCallback) {
    ...
    override fun submitList(list: PagedList<TItem>?) {
...
//        items = list ?: listOf()
    ...
    private fun getItemPositionInCollection(positionInAdapter: Int): Int {
        ...
        return if (shiftedPosition >= 0 && shiftedPosition < super.getItemCount()) shiftedPosition else -1
    }
    ...
    fun getItemViewType(positionInAdapter: Int): Int {
        ...
        val item = if (positionInCollection >= 0) getItem(positionInCollection) else null
    ...
    override fun getItemId(positionInAdapter: Int): Long {
        ...
        tryDelegateAction(positionInAdapter,
                BiConsumer { itemAdapterDelegate, positionInCollection ->
                    result.value = itemAdapterDelegate.getItemId(getItem(positionInCollection)!!,
    ...
    override fun getItemCount(): Int = headersCount + super.getItemCount() + footersCount
    ...
    override fun onBindViewHolder(holder: TItemViewHolder, positionInAdapter: Int) {
        tryDelegateAction(positionInAdapter,
                BiConsumer { itemAdapterDelegate, positionInCollection ->
                    bindItemViewHolder(itemAdapterDelegate, holder, getItem(positionInCollection)!!, null, positionInAdapter, positionInCollection)
                },
                ...
                Consumer { positionInCollection ->
                    if (positionInCollection >= 0) {
                        bindItemViewHolder(null, holder, getItem(positionInCollection)!!, null, positionInAdapter, positionInCollection)
                    }
                })
    }
    ...
    override fun onBindViewHolder(holder: TItemViewHolder, positionInAdapter: Int, payloads: List<Any>) {
        ...
        tryDelegateAction(positionInAdapter,
                BiConsumer { itemAdapterDelegate, positionInCollection ->
                    bindItemViewHolder(itemAdapterDelegate, holder, getItem(positionInCollection)!!,
                            payloads, positionInAdapter, positionInCollection)
                },
                Consumer { positionAdapterDelegate -> positionAdapterDelegate.onBindViewHolder(holder, positionInAdapter) },
                Consumer { positionInCollection ->
                    if (positionInCollection >= 0) {
                        bindItemViewHolder(null, holder, getItem(positionInCollection)!!,
                                payloads, positionInAdapter, positionInCollection)
                    }
                })
    }
    ...
    public override fun getItem(positionInAdapter: Int): TItem? {
        val positionInCollection = getItemPositionInCollection(positionInAdapter)
        return if (positionInCollection >= 0) super.getItem(positionInCollection) else null
    }
